...
private:
    ...
    void InitializeTools() {
        ...
        // 切换到brookesia界面
        mcp_server.AddTool("self.system.goback_to_applications", "返回应用界面：\n"
            "switch: 返回应用界面", 
            PropertyList({
                Property("switch", kPropertyTypeString),
            }), [this](const PropertyList& properties) -> ReturnValue {
                const std::string& action = properties["switch"].value<std::string>();
                ESP_LOGI(TAG, "%s",action.c_str());
                if (action == "返回应用界面" || action == "true" || action == "switch" || action == "back") {
